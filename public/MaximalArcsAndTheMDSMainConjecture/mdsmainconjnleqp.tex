\section{The proof of the MDS main conjecture for $n\leq 2p-2$}\label{sec-msd-nleq2p-2}
...
For convinience we now state a `simplified version' of \person{Segre}'s lemma which is the only version we shall use in the following.
%%%%%
...
\paragraph{First attempt.} The proof of the main conjecture for MDS codes of \person{Ball} and \person{de Beule} for the case where
...
\begin{proof}
    ...
    Thus it follows that also the matrix $N\defeq DM$, where $D\defeq \diag(P(A\setminus\set{a}):a\in A)$, has full rank which is the matrix of the linear system~\theref{mds-class-n-leq-p-keysys}.
