...
\begin{definition}[upper and lower bounds for posets]
    ...
    for the set of lower bounds. These operators correspond to the operators $\bigmeet$ and $\bigjoin$ of the completion lattice of $P$ in the sense that when $\iota:P\to \bar{P}$ is the canonical map to the completion lattice then $\bigjoin\Sub(\iota)(A^{\meet})=\bigmeet\Sub(\iota)(A)$. Moreover, the operators $\meet$ and $\join$ form an antitone \person{Galois} connections $\struct{\struct{\Sub(P),\meet},\struct{\Sub(P),\join}}$ on the subsets of the poset $P$ similarly as $\bigmeet$ and $\bigjoin$ form such connection on the subsets of a complete lattice. 
\end{definition}
...
\begin{remark}
    ...
    $$\bigsetmeet_{i\in I}{A_i^{\meet}}={\left(\bigsetjoin_{i\in I}{A_i}\right)}^{\meet} \textrm{ and } \bigsetmeet_{i\in I}{A_i^{\join}}={\left(\bigsetjoin_{i\in I}{A_i}\right)}^{\join}.$$
\end{remark}
...
\begin{lemma}[bounds for the limit point of a sequence]
    ...
    $$
    a\in {\left(\bigsetjoin_{i\in I}{\set{a_j}_{j \geq i}^{\meet}}\right)}^{\join}\setmeet{\left(\bigsetmeet_{i\in I}\set{a_j}_{j \geq i}^{\join}\right)}^{\meet}
    $$
...
\begin{proof}
    It is clear that for $i\in I$ we have $a\in\set{a_j}_{j \geq i}^{\meet\join}\setmeet\set{a_j}_{j \geq i}^{\join\meet}$ or equivalently for a lattice $\bigmeet\set{a_j}_{j \geq i}\leq a\leq \bigjoin\set{a_j}_{j \geq i}$ since otherwise
    $P\setminus\set{a_j}_{j \geq i}^{\join\meet}$ or $P\setminus\set{a_j}_{j \geq i}^{\join\meet}$ would be an open neighbourhood of $a$ not containing a point of $\set{a_j}_{j\geq i}$ (contradicting the assumption that $a$ is an accumulation point of $N$).
    Thus we have that
    $$
    a\in \bigsetmeet_{i\in I}{\set{a_j}_{j \geq i}^{\meet\join}}\setmeet\bigsetmeet_{i\in I}\set{a_j}_{j \geq i}^{\join\meet}=
    {\left(\bigsetjoin_{i\in I}{\set{a_j}_{j \geq i}^{\meet}}\right)}^{\join}\setmeet{\left(\bigsetmeet_{i\in I}\set{a_j}_{j \geq i}^{\join}\right)}^{\meet},
    $$
...
\begin{definition}[limes superior and inferior]
    ...
    $$
    \sup{\left(\bigsetjoin_{i\in I}{\set{a_j}_{j \geq i}^{\meet}}\right)}=\hat{N}
    $$
    or
    $$
    \inf{\left(\bigsetjoin_{i\in I}{\set{a_j}_{j \geq i}^{\meet}}\right)}=\check{N}
    $$
...
\begin{proof}
    Let $x\in L$ then since $\tau$ is $T_1$ the set $\set{x}$ is closed. Define $f:=(c_x\tuple\id_L)\join$ where $c_x$ is the constant map then $f$ is continuous. Thus, $f^{-1}{\set{x}}=\set{x}^{\join}$ is closed. This implies that for all $A\setleq L$ the set $A^{\join}=\bigsetmeet_{a\in A}{\set{a}^{\join}}$ is closed. For $\meet$ the dual argument works.
...
\begin{proof}
    ...
    Let $a$ an accumulation point of $N$. Then $a$ is an upper bound of $\set{a_i}_{i\in I}$ for otherwise there would exist an $i\in I$ such that $a_i\not\leq a$ implying that $P\setminus \set{a_i}^{\join}$ is a neighbourhood of $a$ not containing a point of $\net[j\geq i]{a_j}$ contradicting the assumption that $a$ is an accumulation point of $N$. Moreover, if there exists an $a'<a$ which is also an upper bound of $\set{a_i}_{i\in I}$ then $P\setminus\set{a'}^{\meet}$ is an open neighbourhood of $a$ not containing a point of $N$. Thus, if $\lim{N}$ exists it must be the supremum of $N$. Conversely, if the supremum of $N$ exists, it must be its limit. This can be seen from as follows. Assume there is an open neighbourhood $U$ of $\sup{N}$ which seperates it from $N$. Then by reducing to the base of topology of the \person{Galois} topology we can assume that $U=P\setminus(\bigsetjoin_{i=1}^n{A_i^{\meet}}\setjoin\bigsetjoin_{i=1}^m{B_i^{\join}})$. When there is an $i\in I$ and $j\in\set{1,\ldots,m}$ such that $a_i\in B_j^{\join}$ then $\sup{N}\in U$. So we may assume that $m=0$. On the other hand, it is easy to see that the sets $A_i^{\meet}\setmeet\set{a_i}_{i\in I}$ are totally ordered by inclusion since $N$ is non-decreasing. Thus there is an $i\in\set{1,\ldots,n}$ such that $\set{a_i}_{i\in I}\setleq A_i^{\meet}$. But then $\sup{N}\in A_i^{\meet}$ by definition of the supremum.
\end{proof}
...
\begin{proof}
    Let a net $\net[i\in I]{a_i}$ converge in $P$ to $a$. Then from $a=\limsup_{i\in I}{a_i}$ we know that for all $a'<a$ there exists an $i\in I$ such that for all $j\geq i$ one of the sets $\set{a_k}_{k\geq j}$ has an upper bound $u$ which is smaller than $a'$.
\end{proof}
...
\begin{eqnarray*}
    S
    &=& \bigsetjoin_{y\in[a,b]}\set{x\in[a,b]:x<y \lgand f(x)<f(y)}\\
    &=& \bigsetjoin_{y\in[a,b]}(\set{x\in[a,b):x<y\}\setmeet\{x\in[a,b):f(x)<f(y)})
\end{eqnarray*}
...
\begin{solution}[\ref{3-cycl-an}]
    ...
    For all subsets $I'\leq I$ such that $M:=\bigsetjoin_{i\in I'}{\spt{c_i}}\subset n$ there exists $j\in I\setminus I'$ such that $\spt{c_j}\setmeet M\neq \emptyset$ and $\spt{c_j}\not\subseteq M$.

    This implies inductively that $\card{\bigsetmeet_{i\in I'}{\spt{c_i}}}\leq 3+2(i-1)$. Thus it follows that $1+2\card{I}\geq n$ must hold. We claim that any set $\set{c_i:i\in I}$ satisfying the above condition necessary for transitity of the action of $\gen{c_i:i\in I}$ generates $A_n$.
    \paragraph{Case 1.}
...
\begin{proof}
    $G$ acts on the set $\cP$ of $p^e$-element subsets of $G$ by right multiplication. One then obtains for some $P\in\cP$ that $\bigsetjoin{P[\stab[G]{P}]}=P$ is a disjoint union of right cosets of $\stab[G]{P}$ yielding that $\stab[G]{P}\divides\card{P}=p^e$. Moreover, $P\in \cP$ is a subgroup of $G$ if and only if $\stab[G]{P}$ has order $p^e$.
...
\begin{lemma}[representation of the generated ideal]
    Let $B\setleq A$ a set. Then the upright ideal generated by $B$ is $\bigsetjoin_{n\in\nats}{{(AB)}^n}$.
\end{lemma}
...
\begin{lemma}[Lebesgue]
    Let $K$ be a compact $A$-metric space and $\cU$ be an open cover of $K$, i.e. $\bigsetjoin\cU=K$. Then there exists an $\epsilon>0$ such that for any $k\in K$ the ball $\ball_{\epsilon}(k)$ lies entirely in some set $U\in \cU$.
\end{lemma}
...
\setfont{theorem}{\sf}
\makeatletter
\def\context@a{a b b c}%%
\def\context@b{ab}%
%
%\pushcontextitem{a}{a}

\def\a{\expandafter\b\expandafter\b\expandafter\b\c}
...
\defcounter{dun}
\setcounterstyle{dun}{arabic}
%%COUNTEST:\getcounter{dun}%%%%
...
\def\a{a b c d {e f g}}
%%
POPLAST:\poplast\a(into:)\b%
\a\\\b%%
\makeatother
\setstring{theorem}{Satz}
%\getfont{theorem}
%\getstring{theorem}
%% 
