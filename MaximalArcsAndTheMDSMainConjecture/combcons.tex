\section{Combinatorics of sets in finite projective space}%
\makeatletter%
...
\begin{definition}\label{kmarc-def}
    A $(k,m)$-arc in projective space $P$ is called \keyword[maximal $(k,m)$-arc]{maximal} if it attains the bound of the \autoref{kmarc-size}.
    It is called \keyword[complete $(k,m)$-arc]{complete} if it is maximal with respect to inclusion among all $(k,m)$-arcs in $P$.
\end{definition}
...
\begin{proof}
    Using \autoref{kmarc-size}, we obtain that for any point $p\in \cA\setmeet P'$ all lines through $p$ contain $m$ points showing that $\cA\setmeet P'$ is maximal by that same fact.
\end{proof}
...
\begin{lemma}[dual $(k,m)$-arc]\label{kmarc-dual}
    Let $\cA$ be a maximal $(k,m)$-arc in the plane $\pi$ such that $\cA\not\in\set{\emptyset,\pi}$ and $\cA'$ be the set of external lines of $\cA$. Then $\cA'$ is a maximal $(k',\frac{q}{m})$-arc in the dual plane (in particular $m\divides q$).
\end{lemma}

\begin{proof}
    Let $E_p$ be a set of external lines of $\cA$ intersecting at $p\in\pi\setminus\cA$. Let $L_p$ be the lines incident with $p$.
    Then we have that $\card{L_p\setminus E_p}m=k$ since $\cA$ is maximal (\autoref{kmarc-size}).
    Thus we deduce that
    $$
...
Hence, non-trivial maximal arcs can only exist for $m\divides q$. Indeed, this is the case for $q$ even, as in this case one can construct the so called \person{Denniston} arcs.

\begin{lemma}
    Let $\pi=\P\field{q}^3$ be a Desarguian projective plane of order $q=2^e$. Then there exist maximal $(k,m)$-arcs for each $m\divides q$. 
\end{lemma}

\begin{proof}
    Consider the affine plane $\field{q}^2$ embedded in $\pi$. Since $m\divides q$ there as a subgroup $H\setleq\field{q}$ of order $m$ of the additive group (which is an elementary Abelian $p$-group). Choose an irreducible homogeneous quadratic polynomial $\gamma(X,Y)$ on $\field{q}^2$. Then $\cA:=\set{(x,y)\in\field{q}^2}[\gamma(x,y)\in H]$ defines a maximal $(k,m)$-arc.
    Any affine line which is parameterized by $l:x\mapsto(x,\alpha x+\beta)$ (w.l.o.g.~we can express $y$ as a function of $x$) plugged into the equation $\gamma(X,Y)=aX^2+bXY+cY^2\in H$ gives an equation of the type $(a+b\alpha+c\alpha^2)x^2+b\beta+c\beta^2\in H$ where the coefficient in front of $x$ cannot vanish since otherwise $\gamma$ would admit a projective zero $\gen{(1,\alpha)}$. But this equation clearly has exactly $\card H=m$ solutions (since the \person{Frobenius} map $x\mapsto x^2$ is a bijection). Thus any affine line intersects $\cA$ in $m$ points and the line at infinity does not intersect $\cA$ at all.
\end{proof}

This is the proof from~\cite[p. 120]{handbookincgeokmarcs} with some additional notes.

As a corollary of \autoref{kmarc-dual} it follows that there are no $(q+2)$-arcs in planes of odd order --- a first (however simple) result which hints to the MDS main conjecture.
...
For the sake of completeness, we prove another fact about hyperovals in Desarguian planes.

\begin{lemma}
    Let $\cO$ be a hyperoval in a Desarguian plane $\pi$. Then $\cO$ is $\PGL$-equivalent to some oval admitting a representation\footnote{here by a representation we mean a system of representatives of the one-dimensional subspaces}
    $$
...
\begin{proof}
    It is well-known that the projective general linear group $\PGL(n,\field{q})$ acts transitively on the $(n+1)$-sets in general position of $\P \field{q}^n$. Thus we may assume that the four vectors $e_0,e_1,e_2$ and $e_0+e_1+e_2$ are part of our representation (using an appropriate element of $\PGL(3,\field{q})$).
    Using an appropriate scalar, we can achieve that the first coordinates of all vectors apart from $e_1$ and $e_2$ are one (as they are not zero for otherwise the corresponding vector would be coplanar with $e_1$ and $e_2$). Since otherwise two of these vectors would be coplanar with $e_1$ the third coordinates of them must all be distinct. For the same reason, the second coordinates of two of these vectors must also be distinct else they would be coplanar with $e_2$. Hence, $f$ is defined and a permutation.
    Lastly, evaluating the determinants of of triples of vectors in which $e_1$ and $e_2$ do not occur we obtain
    $$
...
\begin{corollary}[translation hyperovals]
    ...
    \cC:=\set{e_1,e_2}\setjoin\set{\begin{pmatrix} 1\\ z\\ f(z)\end{pmatrix}}[z\in\field{q}],
    $$
    where $f(z)=z^{2^i}$ is a representation of a hyperoval if and only if $i\meet e=1$.
\end{corollary}
...
\begin{corollary}
    The polynomial $f(z)=z^6$ is an $\cO$-polynomial over $\field{q}$ where $q=2^e$ and $e$ is odd.
\end{corollary}
...
\begin{proof}
    ...
    Assume now that $p=x^2+\alpha x+\beta$ factors. Then its zeros must be of the form $a+c$ and $b+c$ (since their sum is $a+b$). Moreover, by \person{Vieta}'s formulae they must satisfy
    $$
