...
\newcommand{\conlt}{\lhd}
\newcommand{\congt}{\rhd}
\def\define#1{\mathrel{\mathrel{\mathop:}\mkern-1.2mu#1}}
...
\def\realnumber{\mathbb{R}} 			% real numbers
\def\naturalnumber{\mathbb{N}} 				% natural numbers
\def\integer{\mathbb{Z}} 				% integers
\def\rationalnumber{\mathbb{Q}}				% rationals
\def\complexnumber{\mathbb{C}}			% complex numbers
...
\def\subinstances#1\in#2{\mathrm{Sub}_{#2}{#1}}
% Gleichheit als Typ
\def\equals{=}
% Gleichheit als Instanzen eines Typs
\def\as#1\equals{\stackrel{#1}{\equiv}}
...
\def\subobject#1\in#2{\mathop{\mathrm{Sub}_{#2}}{#1}}
...
\def\isoto{\leftrightarrow}% iso
\def\to{\rightarrow}% morphism
\def\auto{\cwcirclearrow}
\def\interpret#1\in#2{{#1}_{#2}}

% standard categories
\def\set{\mathbf{\mathrm{Set}}}
\def\topspace{\mathbf{Top}}
\def\lattice{\mathbf{Lat}}
\def\poset{\mathbf{Poset}}
\def\ring{\mathbf{Rng}}
\def\category{\mathbf{Cat}}
\def\group{\mathbf{Grp}}
\def\cyclicgroup{\mathbf{Cyc}}
\def\logic{\mathbf{Log}}

% power the binary operator #1 to #2 
\def\power#1\to#2{#1#2}
\def\power#1\operation#2\exponent#3{{#1}^{{#3}{#2}}}

\def\multinomialcoefficient#1#2{\left(\begin{array}{#1}\\ {#2}\end{array}\right)}
...
\def\hull#1\in#2{\left\langle{#1}\right\rangle_{#2}} 
