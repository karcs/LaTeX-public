...
\begin{abstract}
    We discuss the question if a weakly generic arrangement with given \textsc{Poincaré}-polynomial can exist in $\field{q}^d$ and generalise some results by \textsc{Zaslavsky} about the number of components of the complement of a real arrangement and its discrete analogue for that type of arrangemnts.
...
In this thesis we aim consider several subsets of finite projective spaces admitting certain 'independence relations'.

\subsection{$(k,m)$-arcs in projective planes}

We start with some objects in the preojective plane.

\begin{definition}[$(k,m)$-arc]
    Let $P$ be a projective space of order $q$. Then a set $\cA$ will be called a \new{$(k,m)$-arc} if $\card{\cA}=k$ and any line $l$ of
    $\pi$ intersects $\cA$ in at most $m$ pionts.
\end{definition}

\begin{definition}[$(m,n)$-secant]
    Let $\cS\subseteq P$ be a subset of some projective space $P$. Then a subspace of (projective) dimension $n$ intersecting $\cS$ in exactly $m$ points is called an \new{$(m,n)$-secant} of $\cS$. In the case we are dropping the $n$ we mean a line. Moreover, a $2$-secant will also be denoted as \new{bisecant}, a $1$-secant as \new{tangent} and a $0$-secant as \new{external line}.
\end{definition} 
...
\begin{definition}
    A $(k,m)$-arc in projective space $P$ is called \new{maximal} if it attains the bound of the lemma \ref{kmarcs:lem1}.
    It is called \new{complete} if it is maximal with respect to inclusion among all $(k,m)$-arcs in $P$.
\end{definition}
...
\begin{lemma}[\textsc{Tallini Scafati}, generalized]
...
\begin{conjecture}[main conjecture on MDS codes, $k$-arcs, generic arrangements in $\field{q}^d$]
