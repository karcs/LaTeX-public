...
\usepackage{mathenv}
\usepackage{layer}
%\usepackage{amssymb} % for arrows
%other packages : extpfeil
%\usepackage{diagrams}
% $%%%\usep$ackage{nath}
\begin{document}
\title{Various Notes}
...
\def\b#1{Tschüss #1}
%%%%%%%%%%%%%%%%%%%%% from amssymb
\def\onto{\twoheadrightarrow} %epi
\def\into{\rightarrowtail} %mono

\chapter{Grundlagen}

Zunächst führen wir den Begriff der Gruppe nocheinmal ein

\begin{definition}[Gruppe]
    Eine Gruppe $G$ ist eine Struktur $\Grp=\struct{\compose,^{-1},1}$
\end{definition}




\chapter{Permutationsgruppen und lineare Gruppen}

\section{Primitive und mehrfach transitive Permutationsgruppen}

\section{...}

\section{Primitive Permutationsgruppen mit abelschen Normalteilern}

\begin{lemma}
    Sei $A$ abelsche transitive Untergruppe von $\Aut \Omega$ ($\Omega:\Set$), dann ist der Zentralisator von $A$ in $\Aut \Omega$
    gleich $A$ selbst und $A$ agiert regulär auf $\Omega$.
\end{lemma}

\def\centralizer{\mathrm C}

\begin{proof}
    Da $A$ abelsch ist, gilt $A\into\xleftarrow{}\leftarrow\leftarrow\centralizer_{\Aut\Omega} A$. Weiterhin stellt man fest, dass $\centralizer A$ frei
    agiert, denn falls für $c\in \centralizer A$ gilt $\omega^c=\omega$ für ein $\omega\in\Omega$, dann folgt für $\rho\in\Omega$ wegen Transitivität von $A$ die
    Existenz eines $a\in A$ mit $\omega^a=\rho$, also $\rho^c=\omega^{ac}=\omega^{ca}=\rho$. Also $c=1_{\Aut \Omega}$. Damit folgt,
    dass $\centralizer A$ sowohl transitiv (da $A$ transitiv agiert), als auch frei agiert. Somit $A=\centralizer A$, denn für ein
    Element $c\in\centralizer A$ mit $\omega^c=\rho$ gibt es mindestens ein $a\in A$ mit derselben Eigenschaft, dann gilt aber
    $a=c$, wegen Freiheit. 
\end{proof}

\begin{lemma}[Struktur charakteristisch einfacher Gruppen]
    Sei $G$ charakteristisch einfach mit minimalem Normalteiler $N$. Dann ist $G\iso N^{\prod\kappa}$.
\end{lemma}

\begin{proof}
    Betrachte das orbit $O\defeq \orb_{\Aut G} N$ von $N$ unter Automorphismen von $G$. Die davon erzeugt Gruppe $H\defeq \gen O$
    liegt charakteristisch in $G$ und $1<N\geq H$ zeigt $H=G$. Weiterhin schneiden sich je zwei veschiedene Elemente $N_1,N_2\in O$
    trivial, also $[N_1,N_2]\leq N_1\meet N_2=1$. Damit ist $G$ eine Potenz von $N$. Zu zeigen bleibt noch, dass $N$ einfach
    ist. Jeder Normalteiler von $N$ ist aber auch ein Normalteiler von $G$ (da $G$ Potenz von $N$ ist), also folgt, wegen
    Minimalität von $N$, dass ein solcher entweder $1$ oder $N$ selbst ist, also ist $N$ einfach. 
\end{proof}

\begin{theorem}[Galois]
    Sei $G$ eine Gruppe mit primitiver Aktion auf $\Omega$ mit minimalem Normalteiler $N$. Ist $N$ auflösbar, so gilt:
    \begin{statements}
            \item $N$ agiert regulär und ist elementar-abelsch.
            \item $G=N\stab_G \omega$ für jedes $\omega\in\Omega$ und $N\meet\stab_G\omega=1$.
            \item $N$ ist der einzige minimale Normalteiler.
            \item $\stab_G \omega$ hat keine $p$-Gruppe als Normalteiler.
        \item Ist $G$ auflösbar, so sind alle Komplemente von $N$ konjugiert zueinander in $G$.
    \end{statements}
\end{theorem}
